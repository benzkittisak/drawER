/* ============================================================
   Mock data for drawDB Live prototype
   Attaches window.DB
   ============================================================ */
(function () {
  const users = {
    ...
    leo:   { id: "leo",   name: "LeoFontaine",  short: "LF", color: "var(--u-leo)",   role: "Designer" },
  };

  // table colors
  const C = {
    indigo: "#6366f1", teal: "#0d9488", amber: "#d97706",
    rose: "#e11d48", violet: "#7c3aed", sky: "#0284c7", green: "#16a34a",
  };

  // ---- schema: a SaaS product DB ----
  const tables = [
    {
  ...
  // relationships: [fromTable, fromField] -> [toTable, toField]
  const rels = [
  ...
  // who's editing what (locks)
  const locks = { tasks: "kenji" };

  // live teammate state (cursor target + viewing)
  const liveUsers = [
  ...
  // comment threads pinned to canvas coords
  const comments = [
    {
      id: "c1", x: 905, y: 60, table: "tasks", resolved: false,
      author: "aisha",
      msg: "Should priority be an enum (low/med/high) instead of int2? Easier for non-devs to read.",
      time: "12m", replies: 2,
    },
    {
      id: "c2", x: 610, y: 360, table: "subs", resolved: false,
      author: "maya",
      msg: "We need a trial_ends_at column before launch — adding now.",
      time: "1h", replies: 1,
    },
    {
      id: "c3", x: 300, y: 80, table: "users", resolved: true,
      author: "kenji",
  ...
  window.DB = { users, tables, rels, locks, liveUsers, comments, activity, versions, diagrams, C };
})();
